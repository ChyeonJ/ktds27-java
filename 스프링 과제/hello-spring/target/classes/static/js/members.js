...
$().ready(function () {
  ...
  //
  // 폼이 전송이 될 때 이벤트를 처리
  $("#signVO").on("submit", function (event) {
    ...
    // 정규표현식으로 입력된 비밀번호를 검사해서 틀리거나, 없으면
    ...
    // 이름, 이메일, 비밀번호를 제대로 입력했다 ==> 폼을 전송.
    if ($(".validation-error").length === 0) {
      // $(this).submit() ==> jQuery Event ==> 14번라인 (preventDefault)에서 전송 이벤트가 사라진 이유 때문에 동작되지 않는다.
