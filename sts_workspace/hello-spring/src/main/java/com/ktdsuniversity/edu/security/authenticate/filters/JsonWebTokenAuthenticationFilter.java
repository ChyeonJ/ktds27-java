...
/**
 * 이 클래스의 객체가 Security Filter Chain에 등록되어 인증이 필요한 모든 End Point가 실행되기 전에
 * AuthenticationToken(UsernamePasswordAuthenticationToken)을 생성하도록 하는 필터
 * 
 * HttpServletRequest의 header로 전달된 Authorization에 들어 있는 JWT를 가져와 분석(벅호화 및 검증)을
 * 진행하고 분석된 결과를 AuthenticationToken으로 생성시킨다.
 */ // 애를 상속 받은 클래스는 SecurityFilterChain에 들어갈 수 있음
public class JsonWebTokenAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		// 인터셉터처럼 설명하다면 preHandle

		// Authorization이 존재하는지 확인하는 조건
		// 요청 URL이 "/api/"로 시작하는 경우만 실행
		// 요청 URL 가져오기
		String requestURI = request.getServletPath();
		// http://localhost:8080/api/,,,, 있으면
		// requestURI ==> /api/,,,, 이것만 가져옴
		if (requestURI.startsWith("/api/")) {
			...
			if (!StringUtils.isEmpty(jsonWebToken)) {
				// JWT를 복호화시켜서 email을 가져온다
				String email = null;
				try {
					email = this.jsonWebTokenAuthenticationProvider.decrpytJsonWebToken(jsonWebToken);
		...
		filterChain.doFilter(request, response); // FilterChain에 JsonWebTokenAuthenticationFilter 다음에 필터가 있다면 동작을 시켜라

		// 모든 필터가 동작이 완료되고 Filter Chain의 역순으로 응답이 돌아올 때
		// 이 필터가 해야할 일 작성
		// postHandle
